...
# Table name kept here so _set_last_import can keep last_completed_at in
# sync atomically with the ingest commit. Slot lifecycle is owned by
# core.sync_manager.BaseSyncManager.
...
class HealthRepository:
    ...
    def _set_last_import(self, conn: sqlite3.Connection, batch_ts: int):
        ...
        conn.execute(
            f"UPDATE {SYNC_STATE_TABLE} SET last_completed_at = ? WHERE id = 1",
